...
namespace inery {
   ...
   /**
    * The `inery.msig` system contract allows for creation of proposed transactions which require authorization from a list of accounts, approval of the proposed transactions by those accounts required to approve it, and finally, it also allows the execution of the approved transactions on the blockchain.
    *
    ...
    */
   class [[inery::contract("inery.msig")]] multisig : public contract {
      public:
         ...
         void approve( name proposer, name proposal_name, permission_level level,
                       const inery::binary_extension<inery::checksum256>& proposal_hash );
         /**
          * Revoke proposal
          *
         ...
         using propose_action = inery::action_wrapper<"propose"_n, &multisig::propose>;
         using approve_action = inery::action_wrapper<"approve"_n, &multisig::approve>;
         using unapprove_action = inery::action_wrapper<"unapprove"_n, &multisig::unapprove>;
         using cancel_action = inery::action_wrapper<"cancel"_n, &multisig::cancel>;
         using exec_action = inery::action_wrapper<"exec"_n, &multisig::exec>;
         using invalidate_action = inery::action_wrapper<"invalidate"_n, &multisig::invalidate>;

      private:
         struct [[inery::table]] proposal {
            name                            proposal_name;
         ...
         typedef inery::multi_index< "proposal"_n, proposal > proposals;

         struct [[inery::table]] old_approvals_info {
            name                            proposal_name;
         ...
         typedef inery::multi_index< "approvals"_n, old_approvals_info > old_approvals;
         ...
         struct [[inery::table]] approvals_info {
            uint8_t                 version = 1;
            name                    proposal_name;
         ...
         typedef inery::multi_index< "approvals2"_n, approvals_info > approvals;

         struct [[inery::table]] invalidation {
            name         account;
         ...
         typedef inery::multi_index< "invals"_n, invalidation > invalidations;
   };
} /// namespace inery
